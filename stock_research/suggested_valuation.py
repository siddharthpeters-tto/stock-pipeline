...
def _growth_assumptions(revenue_rows: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    spread = max(0.02, 0.5 * growth_std_dev)
    raw = {
...
def _margin_assumptions(rows: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    raw_base = statistics.median(valid[-3:])
    raw_conservative = _percentile(valid, 0.25)
    raw_optimistic = _percentile(valid, 0.75)
...
def terminal_multiple(revenue_growth: float, roic: Optional[float], fcf_margin: float, net_debt_to_ebitda: Optional[float]) -> Dict[str, float]:
    growth_derived = 12.0 + 0.8 * (revenue_growth * 100.0)
    quality_adjustment = 0.0
    if (roic is not None and roic < 0.08) or (net_debt_to_ebitda is not None and net_debt_to_ebitda > 3.0):
        quality_adjustment = -2.0
    elif roic is not None and roic >= 0.20 and fcf_margin >= 0.15:
        quality_adjustment = 2.0
    return {
        "growth_derived_multiple": growth_derived,
        "quality_adjustment": quality_adjustment,
        "terminal_p_fcf": _clamp(growth_derived + quality_adjustment, 12.0, 30.0),
    }
...
def calculate_suggested_valuation(
    ...
    normalized_roic = _number(roic)
    normalized_leverage = _number(net_debt_to_ebitda)
    ...
    for name in ("conservative", "base", "optimistic"):
        ...
        multiple = terminal_multiple(assumed_growth, normalized_roic, assumed_margin, normalized_leverage)
    ...
    result["methodology"] = {
        ...
        "revenue_growth_method": "70% longest-span annual revenue CAGR + 30% latest YoY growth; base clamped to 0%-20%; scenario spread is max(2pp, 0.5x population standard deviation of valid YoY growth); scenarios clamped to 0%-25%",
        "fcf_margin_method": "base is median of latest 3 valid annual FCF margins; conservative/optimistic are linearly interpolated 25th/75th percentiles of up to 5 valid annual margins; scenarios clamped to 0%-40% and ordered",
        "terminal_multiple_method": "12 + 0.8x assumed revenue growth percentage, adjusted by +2x for ROIC >=20% and FCF margin >=15%, or -2x for ROIC <8% or net debt/EBITDA >3x; final multiple clamped to 12x-30x",
